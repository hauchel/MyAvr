...
void zeigLad() {
  char str[50];
  sprintf(str, "Max   %5d mA", lad.max_mA);
  Serial.println(str);
  sprintf(str, "Max   %5d mV", lad.max_mV);
  Serial.println(str);
  sprintf(str, "ZlOff %5d mV", lad.llzOff_mV);
  Serial.println(str);
  sprintf(str, "ZlOn  %5d mV", lad.llzOn_mV);
  Serial.println(str);
  sprintf(str, "Lade %5d mA", lad.lade_mA);
  Serial.println(str);
  sprintf(str, "Keep %5d mA", lad.keep_mA);
  Serial.println(str);
}
...
void readIna() {
  ...
  sprintf(str, "Out: %4u, Volt: %5d  Curr: %5d", outV, busvolt_mV, current_mA);
  Serial.print(str);
  vt100ClrEol();
  Serial.println();
  lc.home();
...
void kennlinie(int og) {
  // Ändert DAC und liest Ina:
  int delt = 50;
  outV = 0;
  for (byte i = 0; i < 10; i++ ) {
    chng(0);
...
void regel() {
  ...
  sprintf(str, "Soll %4d  delt %+4d  reg %4d %c", soll_mA, dlt, reg, ov);
  Serial.println(str);
  chng(reg);
}
