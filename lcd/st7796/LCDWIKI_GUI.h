...
class LCDWIKI_GUI {
	...
	protected:
		int16_t text_x; // the text cursor's x coordinate
		int16_t text_y; // the text cursor's y coordinate
		uint16_t text_color; // the foreground colour of the text
		uint16_t text_bgcolor; // the background colour of the text
		uint16_t draw_color; // the colour when drawing  pixels
		uint8_t text_size; // the text scaling size
